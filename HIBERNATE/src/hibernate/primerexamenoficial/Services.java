...
import hibernateDAOServices.Sesion;
...
public class Services {
	...
	private DepartmentsDAO departmentsDAO=null;//atributo de la clase departamentosDAO
	...
	public Services(){
		...
		//En entornos profesionales en este punto añadimos la interfas CRUD para
		//hacerlo mas generico y compatible con todas las bases de datos pero en esta 
		//ocacion lo vamos a dejar pasar aunque en futuros proyectos intentaremos
	...
	public boolean incrementarSalario(){
		...
		Session sesion = null;
		sesion = sm.obtenerSesionNueva();//asignamos la sesion del sesionManager.
		List <Employees>lsEmDTO = null;//asignamos una array donde vamos a almacenar los empleados.
		...
		try{
			...
			lsEmDTO = empleadoDAO.readAll();//almacenamos en el array la lista recogida de la BD.
			actualizarSalario(lsEmDTO);//leemos de nuevo la array pero con los datos actualizados.
			transaction.commit();//verificado que todo funciona correctamente hacemos que los datos se vuelquen a la BD original y el estado del objeto cambie a persistent.
			
			salarioincrementado = true;//decimos que es true cuando los cambios se han ralizado correctamente.
			
		}catch(Exception e){
			e.printStackTrace();
			transaction.rollback();//si la transaccion falla volveremos al punto anterio y deshacemos los cambios realizados.
	...
	private void actualizarSalario(List<Employees> lsEmDTO) {
		...
		// Recorremos la array de empleados para actualizar los datos antes de que sean persistentes en la BD por medio de un for edge.
		
		for (Employees employees : lsEmDTO){
			
			System.out.println("salario inicial" + employees.getSalary());
			
			//realizamos la operacion de multiplicarpor 1,20 para incrementar el salario un 20%
			
			employees.setSalary(employees.getSalary().multiply(new BigDecimal(1.2)));//creamos un objeto de bigdecimal con el valor de 1.2 para realizar la operacion.
			System.out.println("Salario incrementado" + employees.getSalary());//imprimos el resultado para comrpobar que se ha incrementado correctamente.
	...
	public List<Employees> listarEmpleadosQueMasGanan ()
	{
		...
		try
		{
			...
			empleadoDAO.setSesion(sesion);
			transaction = empleadoDAO.getSesion().beginTransaction();
			lsDptos = leerTodosLosDepartamentos(); //recuperar lista de departamentos
			for (Departments dpto : lsDptos)//recorrer lista de departamentos recogida
			{
	...
	private Employees obtenerElQueMasGana(Departments dpto){
		...
		//Creamos el objeto que vamos a comparar para que guarde el mejor sueldo por dpto.
		...
		for (Employees employee : ls){
			...
			//Con intValue cambiamos el valor de BigDecimal a int para poder operar.
			if (employee.getSalary().intValue() > mejorSueldo.intValue()){
				
				mejorSueldo = employee.getSalary();//asignamos el salario a mejorSueldo.
	...
	private List<Departments> leerTodosLosDepartamentos(){
		...
		//Con readAll leemos todo de la misma categoria, en este caso son departamentos pero lo podemos aplicar a trabajos, empleados, etc.
	...
	/**
	 * Metodo para obtener empleados de un departamento dada la ID del departamento.	
	 * @param department
	...
	public List<Employees> obtenerEmpleadosPorDepartamento (Object department){
		...
		try{
			...
			transaction= empleadoDAO.getSesion().beginTransaction();//no se porque me obliga a hacer cast.
