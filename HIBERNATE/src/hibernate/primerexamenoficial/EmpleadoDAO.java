...
public class EmpleadoDAO extends SuperClassDAO implements Iemployees {
	...
	public Object read(Object Id) {
		...
		//Le asignamos una sesion y una sentencia que en este caso es leer empleado dada su ID(cuando busque el empleado debe de darnos un unico resultado).
		em = (Employees)sm.obtenerSesionNueva().createSQLQuery(InstruccionesSQL.COSULTAR_EMPLEADO_X_ID).addEntity(Employees.class).uniqueResult();
	...
	public List<Employees> readAll() {
		...
		ls =  ((SQLQuery) sm.obtenerSesionNueva().createQuery(InstruccionesSQL.CONSULTAR_TODOS)).addEntity(Employees.class).list();//no creo que funcione, revisar
	...
	public List<Employees> listadoPorDepartamento(Object department) {
		...
		ls =  ((SQLQuery) sm.obtenerSesionNueva().createQuery(InstruccionesSQL.CONSULTAR_EMPLEADOS_X_DEPARTAMENTOS + department)).addEntity(Employees.class).list();
